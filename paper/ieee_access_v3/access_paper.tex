...
$[74.92^\circ, 13.00^\circ]$. Table~\ref{tab:study} summarizes the physical
...
Modeling bounding box & $74.65^\circ$--$75.40^\circ$E, $12.65^\circ$--$13.15^\circ$N & -- \\
...
Hospitals modeled & 7 & OpenStreetMap \\
Incident sites modeled & 7 & this study \\
\bottomrule
...
(b) Karnataka with the Dakshina Kannada modeling box. (c) The Mangaluru
...
\label{eq:freq}
\end{equation}
Vectorizing $\Phi>0$ over the routing window yields 201 polygons totalling
...
\emph{only} on the \num{2640} labeled points outside the Mangaluru window,
...
deployment to a neighboring taluk would demand.
...
of the 34.3\% positive fraction is simply the balanced-prior artifact. We
...
labeled points \emph{outside} the Mangaluru window and scored against SAR
...
This is a structural property of the network rather than a limitation of the
router, and it carries the most direct operational implication in this study: in
this city, risk-aware routing can reduce average exposure substantially but
cannot avoid the chokepoints, so resilience investment should target crossing
redundancy rather than routing software.
...
single-pair improvements in this literature are frequently selected favorably.

\paragraph{The result is not an artifact of the chosen incident sites}
...
indifferent to it across a wide neighborhood.
...
neighboring the operating point falls from 15.9\% to 2.4\%, so the
...
inherited as a modeling convention.
...
of the verified admissibility of~\eqref{eq:heuristic}. A$^\ast$ settled
...
a circuitous approach and straight-line distance is a poor proxy for road
...
saturates: essentially all of it is realized by $p=3$, and the $p=4\to5$
...
COBYLA is a local optimizer on a non-convex landscape and the INTERP warm start
...
\item \textbf{Flood-aware routing yields a 33.0\% reduction in mean exposure for a
