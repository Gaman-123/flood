...
\title{\bfseries A Two-Stage Spatiotemporal Flood-Risk Framework with
Flood-Aware Emergency Routing and Hybrid Quantum Dispatch for a
Single-Orbit Coastal District}
...
\begin{itemize}
\itemsep1pt
\item[\textbf{RQ1}] How much of the apparent skill of a flood-susceptibility
      model in this setting survives the removal of remote-sensing circularity
      and spatial leakage?
\item[\textbf{RQ2}] Can a temporal trigger constructed only from published
      rainfall thresholds, with no fitting to observed events, order documented
      floods correctly, and where does its discrimination fail?
\item[\textbf{RQ3}] What does flood-aware routing actually buy in exposure
      reduction per minute of detour, and how sensitive is that trade to the
...
\begin{tikzpicture}[
  ...
  boxq/.style={box, fill=orange!14},
  boxv/.style={box, fill=green!10},
...
\SI{3.0}{\kilo\metre\squared} (Figure~\ref{fig:sar_inventory}). This area is a
...
polygons, \SI{3.0}{\kilo\metre\squared}, on a HAND backdrop with the channel
...
\SI{12.0}{\minute}. We report the all-pairs figure as the headline because
...
impassability threshold $\tau$, over all 49 hospital-incident pairs at
$T=0.70$. (a) Mean route exposure, (b) mean detour, (c) severed pairs. Vertical
banding shows the outcome depends on $\tau$ far more than on $\kappa$; the red
box marks the operating point.}
...
\item \textbf{A trigger built only from published rainfall thresholds orders
      ...
      72-hour saturation term as the variable carrying the real signal (RQ2).
\item \textbf{Flood-aware routing buys a 34.5\% reduction in mean exposure for a
      3.8-minute mean detour, and the binding constraint is network topology, not
      the cost function.} Across 42 pairs mean exposure falls from 0.175 to
      0.114. Maximum exposure barely moves, because all routes share a small set
      ...
      by policy on acceptable risk, not by convention (RQ3).
\item \textbf{Search-transparent benchmarking exposes trade-offs that single
      ...
      layer reproduces the Hungarian optimum exactly and improves monotonically
      in depth over five seeds, at a problem size where it offers no advantage.
\end{enumerate}
...
figure was optimistic. We think reporting that sequence is more useful to a
practitioner deciding whether to trust such a map than reporting only its first
term.
...
Institutional acknowledgments are withheld for anonymous review. The authors
thank the Copernicus, USGS, CHIRPS, MERIT Hydro, Natural Earth and OpenStreetMap
communities for making the data underlying this study openly available.
